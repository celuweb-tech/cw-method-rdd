...
/**
 * Detecta si el proyecto frontend usa Next.js
 */
function detectNextJs(projectRoot) {
  try {
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      // Verificar dependencia de Next.js
      if (deps.next) {
        return true;
      }
    }
    
    // Verificar existencia de archivos de configuración de Next.js
...
async function installRules(options = {}) {
  try {
    ...
    // Detectar si es un proyecto Next.js (después de determinar projectType)
    const isNextJs = projectType === 'frontend' && detectNextJs(projectRoot);

    // Determinar nombre para mostrar (ej: React en lugar de frontend)
    let displayName = projectType;
    if (projectType === 'frontend') {
      try {
        const pkg = require(path.join(projectRoot, 'package.json'));
        const deps = { ...pkg.dependencies, ...pkg.devDependencies };
        // Priorizar Next.js sobre React
        if (isNextJs || deps.next) displayName = 'Next.js';
        else if (deps.react || deps['react-dom']) displayName = 'React';
        else if (deps.vue || deps.nuxt) displayName = 'Vue';
        else if (deps['@angular/core']) displayName = 'Angular';
        else if (deps.svelte || deps['@sveltejs/kit']) displayName = 'Svelte';
      } catch (e) { }
    }

    console.log(chalk.cyan(`\n📦 Tipo de proyecto: ${chalk.bold(displayName)}\n`));
    ...
    // Si es Next.js, primero obtener la lista de archivos de Next.js para excluirlos de las reglas generales
    let nextJsFiles = [];
    if (isNextJs) {
      const nextRulesPath = path.join(rulesSource, projectType, 'next');
      if (fs.existsSync(nextRulesPath)) {
        const nextFiles = await fs.readdir(nextRulesPath);
        nextJsFiles = nextFiles.filter(file => file.endsWith('.mdc'));
      }
    }
    ...
    if (rulesToInstall.includes(projectType)) {
      ...
      if (fs.existsSync(frontRulesPath)) {
        ...
        for (const file of files) {
          // Omitir carpetas (como 'next') ya que se manejan por separado
          const filePath = path.join(frontRulesPath, file);
          const stat = await fs.stat(filePath);
          
          // Si es Next.js y este archivo existe en la carpeta next, omitirlo
          if (isNextJs && nextJsFiles.includes(file)) {
            continue;
          }
          
          if (stat.isFile() && file.endsWith('.mdc')) {
            await fs.copy(
              filePath,
        ...
        installationMessages.push(chalk.green(`✅ ${frontRulesCount} reglas de ${displayName} instaladas`));
      } else {
        installationMessages.push(chalk.yellow(`⚠️  No se encontraron reglas para ${projectType}`));
      }
    }

    // Si es Next.js, instalar reglas específicas de Next.js (reemplazan a las generales)
    if (isNextJs) {
      const nextRulesPath = path.join(rulesSource, projectType, 'next');
      if (fs.existsSync(nextRulesPath)) {
        let nextRulesCount = 0;
        for (const file of nextJsFiles) {
          // Copiar sin prefijo ya que reemplazan a las reglas generales
          await fs.copy(
            path.join(nextRulesPath, file),
            path.join(cursorRulesDir, file)
          );
          installedCount++;
          nextRulesCount++;
        ...
        if (nextRulesCount > 0) {
          installationMessages.push(chalk.green(`✅ ${nextRulesCount} reglas específicas de Next.js instaladas (reemplazan reglas generales)`));
        }
      }
    }
    ...
    if (!fs.existsSync(hooksPath)) {
      try {
        ...
        if (fs.existsSync(projectHooksPath)) {
          ...
          installationMessages.push(chalk.green(`✅ Hooks específicos para ${displayName} detectados`));
        } else {
